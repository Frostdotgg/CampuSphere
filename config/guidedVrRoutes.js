"use strict";

/* ========================================
   CampuSphere — Guided VR route catalog (BE.4)

   Frozen, source-controlled catalog of guided VR walkthroughs that are
   defined by STABLE NATURAL IDENTITIES only:

     - destination_name       canonical building display name (matched via
                              services/routeAvailability.canonicalKey by the
                              caller — this module performs no normalization
                              of its own so there is exactly ONE canonical-name
                              function in the codebase);
     - destination_node_key   the route-graph node the walkthrough must end at;
     - arrival_scene_key      the ONLY scene whose presence as the final
                              verified step counts as truthful arrival;
     - scene_keys             the owner-approved ordered panorama sequence.
     - allowed_missing_link_pairs (optional) names explicit adjacent scene
                              pairs that intentionally have no stored
                              hotspot link; every other pair remains strict.
     - allow_unmapped_start (optional) permits a first panorama whose stored
                              node_id is null; the route graph still validates
                              the destination node.

   HARD RULES:
   - Natural keys only. This module must NEVER contain backend numeric ids
     (vr_scenes.id, buildings.id, route_nodes.id are backend-local and must
     never be compared or copied across MySQL/Supabase).
   - No imports, no env reads, no side effects — pure frozen data, safe to
     require from any layer.
   - The catalog describes the GUIDED walkthrough only. Post-arrival interior
     scenes (e.g. scene-cas-1st-floor-2 / -3) are deliberately NOT steps; they
     stay reachable through the arrival scene's own hotspots in Free Roam.
   - Adding a destination here does not by itself make it arrivable: the
     runtime still verifies every scene key and every forward/reverse
     directional link not explicitly allowlisted above in the ACTIVE VR data
     source, and truthfully reports partial coverage when the chain is
     incomplete.
   - A destination listed in DEFERRED_GUIDED_VR_DESTINATIONS remains routable
     on the campus map, but guided VR must fail closed with an explicit
     unavailable state until the owner supplies and approves its panorama
     sequence. A destination must never appear in both collections.
   ======================================== */

const VEHICLE_GUIDED_VR_ROUTES = Object.freeze([
  Object.freeze({
    destination_name: "Academic Building III",
    destination_node_key: "acad-3",
    arrival_scene_key: "scene-cas-1st-floor",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-39",
      "scene-cas-1st-floor",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building IV",
    destination_node_key: "ccs",
    arrival_scene_key: "scene-ccs-1st-floor",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-85",
      "scene-general-road-94",
      "scene-ccs-1st-floor",
    ]),
  }),

  Object.freeze({
    destination_name: "Academic Building VI",
    destination_node_key: "acad-6",
    arrival_scene_key: "scene-chs-1st-floor-001",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-85",
      "scene-general-road-94",
      "scene-general-road-93",
      "scene-general-road-92",
      "scene-general-road-91",
      "scene-chs-1st-floor-001",
    ]),
  }),

  Object.freeze({
    destination_name: "Academic Building V",
    destination_node_key: "acad-5",
    arrival_scene_key: "scene-acad-5-1st-floor-7",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-39",
      "scene-general-road-41",
      "scene-general-road-42",
      "scene-general-road-43",
      "scene-general-road-44",
      "scene-general-road-45",
      "scene-general-road-46",
      "scene-general-road-47",
      "scene-general-road-48",
      "scene-general-road-49",
      "scene-acad-5-1st-floor-7",
    ]),
  }),

  Object.freeze({
    destination_name: "Green Building",
    destination_node_key: "green",
    arrival_scene_key: "scene-green-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-green-1st-floor-1",
    ]),
  }),

  Object.freeze({
    destination_name: "Supply & Property Building",
    destination_node_key: "supply-property-bldg",
    arrival_scene_key: "scene-supply-1st-floor-001",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-supply-1st-floor-001",
    ]),
  }),

  Object.freeze({
    destination_name: "Academic Building II",
    destination_node_key: "acad-2",
    arrival_scene_key: "scene-acad-2-1st-floor-17",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-54",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-acad-2-1st-floor-17",
    ]),
  }),

  Object.freeze({
    destination_name: "CITD Building",
    destination_node_key: "citd",
    arrival_scene_key: "scene-citd-1st-floor-5",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-citd-1st-floor-5",
    ]),
  }),

  Object.freeze({
    destination_name: "MULTI-PURPOSE-BUILDING I",
    destination_node_key: "multi-1",
    arrival_scene_key: "scene-audit-building-006",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-27",
      "scene-general-road-28",
      "scene-general-road-29",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-54",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-audit-building-006",
    ]),
  }),
  Object.freeze({
    destination_name: "Gymnasium",
    destination_node_key: "gym",
    arrival_scene_key: "scene-gym-1st-floor-bleacher-1",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-gym-1st-floor-bleacher-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Villafuerte Hall",
    destination_node_key: "villafuerte-hall",
    arrival_scene_key: "scene-vh-1",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-pearl-park-2",
      "scene-pearl-park-17",
      "scene-vh-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Pearl Park",
    destination_node_key: "pearl-park",
    arrival_scene_key: "scene-pearl-park-2",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-pearl-park-2",
    ]),
  }),

  Object.freeze({
    destination_name: "Laboratory & Shop Building",
    destination_node_key: "lab-shop",
    arrival_scene_key: "scene-lab-and-shop-building-001",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-lab-and-shop-building-001",
    ]),
  }),

  Object.freeze({
    destination_name: "Graduate School Building",
    destination_node_key: "graduate",
    arrival_scene_key: "scene-graduate-school-1st-floor-4",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-graduate-school-1st-floor-4",
    ]),
  }),

  Object.freeze({
    destination_name: "Technohub Building",
    destination_node_key: "techno-bldg",
    arrival_scene_key: "scene-techno-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-techno-1st-floor-1",
    ]),
  }),

  Object.freeze({
    destination_name: "Academic Building I",
    destination_node_key: "acad-1",
    arrival_scene_key: "scene-acad1-1st-floor-4",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-acad1-1st-floor-4",
    ]),
  }),

  Object.freeze({
    destination_name: "Multi-Purpose Building II",
    destination_node_key: "multi-2",
    arrival_scene_key: "scene-multi-2-1st-floor-3",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-multi-2-1st-floor-3",
    ]),
  }),

  Object.freeze({
    destination_name: "College Dormitory",
    destination_node_key: "dorm",
    arrival_scene_key: "scene-dorm-build-001",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-audit-building-017",
      "scene-audit-building-018",
      "scene-csc",
      "scene-dorm-build-001",
    ]),
  }),

  Object.freeze({
    destination_name: "Central Student Council",
    destination_node_key: "csc",
    arrival_scene_key: "scene-csc",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-audit-building-017",
      "scene-audit-building-018",
      "scene-csc",
    ]),
  }),

  Object.freeze({
    destination_name: "Library Building",
    destination_node_key: "library",
    arrival_scene_key: "scene-library-1st-floor-4",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-library-1st-floor-4",
    ]),
  }),

  Object.freeze({
    destination_name: "FOOD LABORATORY BUILDING",
    destination_node_key: "food",
    arrival_scene_key: "scene-foodlab-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-foodlab-1st-floor-1",
    ]),
  }),

  Object.freeze({
    destination_name: "Administration Building",
    destination_node_key: "admin-bldg",
    arrival_scene_key: "scene-admin-1st-floor-3",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-duran-1st-floor-13",
      "scene-general-road-19",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-admin-1st-floor-3",
    ]),
  }),

  Object.freeze({
    destination_name: "Duran Hall",
    destination_node_key: "duran",
    arrival_scene_key: "scene-duran-1st-floor-6",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-general-road-16",
      "scene-general-road-17",
      "scene-duran-1st-floor-6",
    ]),
  }),

  Object.freeze({
    destination_name: "Freedom Park",
    destination_node_key: "free-park",
    arrival_scene_key: "scene-freedom-park-3",
    scene_keys: Object.freeze([
      "scene-guard-house",
      "scene-general-road-10",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-general-road-16",
      "scene-general-road-17",
      "scene-freedom-park-3",
    ]),
  }),

  Object.freeze({
    destination_name: "Staff House",
    destination_node_key: "staff-house",
    arrival_scene_key: "scene-staff-house-2nd-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-guard-staff-house-2nd-floor-3",
      "scene-guard-staff-house-2nd-floor-4",
      "scene-staff-house-2nd-floor-1",
    ]),
  }),
]);

/*
 * Direction-specific Vehicle exits are kept separate from the entrance
 * catalog.  The scene_keys below are already ordered from the destination to
 * the Guard House / Main Gate; the resolver must not reverse this sequence.
 * `reverse_exit: false` is intentionally explicit so the existing Walking
 * catalog can continue to use its copied-and-reversed exit behavior.
 */
const VEHICLE_EXIT_GUIDED_VR_ROUTES = Object.freeze([
  Object.freeze({
    destination_name: "Academic Building III",
    destination_node_key: "acad-3",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-cas-1st-floor",
      "scene-general-road-39",
      "scene-general-road-41",
      "scene-general-road-42",
      "scene-general-road-43",
      "scene-general-road-44",
      "scene-general-road-45",
      "scene-general-road-46",
      "scene-general-road-47",
      "scene-general-road-48",
      "scene-general-road-49",
      "scene-general-road-50",
      "scene-general-road-51",
      "scene-general-road-52",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building IV",
    destination_node_key: "ccs",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-ccs-1st-floor",
      "scene-general-road-94",
      "scene-general-road-93",
      "scene-general-road-92",
      "scene-general-road-91",
      "scene-general-road-90",
      "scene-general-road-89",
      "scene-general-road-88",
      "scene-general-road-87",
      "scene-general-road-86",
      "scene-general-road-85",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-54",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building VI",
    destination_node_key: "acad-6",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-chs-1st-floor-001",
      "scene-general-road-91",
      "scene-general-road-90",
      "scene-general-road-89",
      "scene-general-road-88",
      "scene-general-road-87",
      "scene-general-road-86",
      "scene-general-road-85",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-54",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building V",
    destination_node_key: "acad-5",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-acad-5-1st-floor-7",
      "scene-general-road-49",
      "scene-general-road-50",
      "scene-general-road-51",
      "scene-general-road-52",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Green Building",
    destination_node_key: "green",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-green-1st-floor-1",
      "scene-general-road-38-5",
      "scene-general-road-54",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Supply & Property Building",
    destination_node_key: "supply-property-bldg",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-supply-1st-floor-001",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-54",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building II",
    destination_node_key: "acad-2",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-acad-2-1st-floor-17",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "CITD Building",
    destination_node_key: "citd",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-citd-1st-floor-5",
      "scene-general-road-30",
      "scene-general-road-31",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-54",
      "scene-general-road-53",
      "scene-general-road-57",
      "scene-general-road-58",
      "scene-general-road-59",
      "scene-general-road-60",
      "scene-general-road-61",
      "scene-general-road-62",
      "scene-general-road-63",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "MULTI-PURPOSE-BUILDING I",
    destination_node_key: "multi-1",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-audit-building-006",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Gymnasium",
    destination_node_key: "gym",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-gym-1st-floor-bleacher-1",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Villafuerte Hall",
    destination_node_key: "villafuerte-hall",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-vh-1",
      "scene-pearl-park-17",
      "scene-pearl-park-16",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Pearl Park",
    destination_node_key: "pearl-park",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-pearl-park-2",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Laboratory & Shop Building",
    destination_node_key: "lab-shop",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-lab-and-shop-building-001",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Graduate School Building",
    destination_node_key: "graduate",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-graduate-school-1st-floor-4",
      "scene-general-road-20",
      "scene-general-road-21",
      "scene-general-road-22",
      "scene-general-road-23",
      "scene-general-road-24",
      "scene-general-road-25",
      "scene-general-road-26",
      "scene-general-road-65",
      "scene-general-road-66",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Technohub Building",
    destination_node_key: "techno-bldg",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-techno-1st-floor-1",
      "scene-duran-1st-floor-13",
      "scene-general-road-15",
      "scene-general-road-16",
      "scene-general-road-17",
      "scene-general-road-18",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building I",
    destination_node_key: "acad-1",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-acad1-1st-floor-4",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-general-road-16",
      "scene-general-road-17",
      "scene-general-road-18",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Multi-Purpose Building II",
    destination_node_key: "multi-2",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-multi-2-1st-floor-3",
      "scene-general-road-11",
      "scene-general-road-12",
      "scene-general-road-13",
      "scene-general-road-14",
      "scene-general-road-15",
      "scene-general-road-16",
      "scene-general-road-17",
      "scene-general-road-18",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "College Dormitory",
    destination_node_key: "dorm",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-dorm-build-001",
      "scene-csc",
      "scene-audit-building-018",
      "scene-audit-building-017",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Central Student Council",
    destination_node_key: "csc",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-csc",
      "scene-audit-building-018",
      "scene-audit-building-017",
      "scene-general-road-67",
      "scene-general-road-68",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Library Building",
    destination_node_key: "library",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-library-1st-floor-4",
      "scene-general-road-69",
      "scene-general-road-70",
      "scene-general-road-71",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "FOOD LABORATORY BUILDING",
    destination_node_key: "food",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-foodlab-1st-floor-1",
      "scene-general-road-72",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Administration Building",
    destination_node_key: "admin-bldg",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-admin-1st-floor-3",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Duran Hall",
    destination_node_key: "duran",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-duran-1st-floor-6",
      "scene-general-road-17",
      "scene-general-road-18",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Freedom Park",
    destination_node_key: "free-park",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    scene_keys: Object.freeze([
      "scene-freedom-park-3",
      "scene-general-road-17",
      "scene-general-road-18",
      "scene-general-road-73",
      "scene-general-road-74",
      "scene-general-road-75",
      "scene-general-road-76",
      "scene-general-road-77",
      "scene-general-road-78",
      "scene-general-road-80",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
  Object.freeze({
    destination_name: "Staff House",
    destination_node_key: "staff-house",
    arrival_scene_key: "scene-guard-house",
    reverse_exit: false,
    allow_unmapped_start: true,
    scene_keys: Object.freeze([
      "scene-staff-house-1st-floor-2",
      "scene-general-road-83",
      "scene-general-road-84",
      "scene-general-road-10",
      "scene-guard-house",
    ]),
  }),
]);

/*
 * Walking routes are intentionally kept in a separate catalog.  The existing
 * vehicle catalog remains the compatibility default for every destination;
 * adding a walking route never changes a vehicle scene sequence.
 */
const WALKING_GUIDED_VR_ROUTES = Object.freeze([
  Object.freeze({
    destination_name: "Academic Building IV",
    destination_node_key: "ccs",
    arrival_scene_key: "scene-ccs-1st-floor",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
      "scene-graduate-school-1st-floor-2",
      "scene-graduate-school-1st-floor-3",
      "scene-graduate-school-1st-floor-4",
      "scene-graduate-school-1st-floor-5",
      "scene-lab-and-shop-building-001",
      "scene-lab-and-shop-building-002",
      "scene-lab-and-shop-building-003",
      "scene-lab-and-shop-building-004",
      "scene-lab-and-shop-building-005",
      "scene-lab-and-shop-building-006",
      "scene-lab-and-shop-building-007",
      "scene-lab-and-shop-building-008",
      "scene-lab-and-shop-building-009",
      "scene-lab-and-shop-building-010",
      "scene-lab-and-shop-building-011",
      "scene-lab-and-shop-building-012",
      "scene-lab-and-shop-building-013",
      "scene-citd-1st-floor-1",
      "scene-citd-1st-floor-2",
      "scene-citd-1st-floor-3",
      "scene-citd-1st-floor-4",
      "scene-citd-1st-floor-5",
      "scene-citd-1st-floor-6",
      "scene-citd-1st-floor-7",
      "scene-citd-1st-floor-8",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-85",
      "scene-general-road-94",
      "scene-ccs-1st-floor",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building II",
    destination_node_key: "acad-2",
    arrival_scene_key: "scene-acad-2-1st-floor-17",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-016",
      "scene-audit-building-015",
      "scene-audit-building-014",
      "scene-audit-building-013",
      "scene-audit-building-012",
      "scene-audit-building-006",
      "scene-audit-building-005",
      "scene-audit-building-004",
      "scene-audit-building-003",
      "scene-audit-building-002",
      "scene-audit-building-001",
      "scene-acad-2-1st-floor-15",
      "scene-acad-2-1st-floor-20",
      "scene-acad-2-1st-floor-19",
      "scene-acad-2-1st-floor-18",
      "scene-acad-2-1st-floor-16",
      "scene-acad-2-1st-floor-17",
    ]),
  }),
  Object.freeze({
    destination_name: "MULTI-PURPOSE-BUILDING I",
    destination_node_key: "multi-1",
    arrival_scene_key: "scene-audit-building-006",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-016",
      "scene-audit-building-015",
      "scene-audit-building-014",
      "scene-audit-building-013",
      "scene-audit-building-012",
      "scene-audit-building-006",
    ]),
  }),
  Object.freeze({
    destination_name: "Central Student Council",
    destination_node_key: "csc",
    arrival_scene_key: "scene-csc",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-018",
      "scene-csc",
    ]),
  }),
  Object.freeze({
    destination_name: "College Dormitory",
    destination_node_key: "dorm",
    arrival_scene_key: "scene-dorm-build-001",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-018",
      "scene-csc",
      "scene-dorm-build-001",
    ]),
  }),
  Object.freeze({
    destination_name: "Library Building",
    destination_node_key: "library",
    arrival_scene_key: "scene-library-1st-floor-4",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
    ]),
  }),
  Object.freeze({
    destination_name: "FOOD LABORATORY BUILDING",
    destination_node_key: "food",
    arrival_scene_key: "scene-foodlab-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Administration Building",
    destination_node_key: "admin-bldg",
    arrival_scene_key: "scene-admin-1st-floor-3",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building VI",
    destination_node_key: "acad-6",
    arrival_scene_key: "scene-chs-1st-floor-001",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
      "scene-graduate-school-1st-floor-2",
      "scene-graduate-school-1st-floor-3",
      "scene-graduate-school-1st-floor-4",
      "scene-graduate-school-1st-floor-5",
      "scene-lab-and-shop-building-001",
      "scene-lab-and-shop-building-002",
      "scene-lab-and-shop-building-003",
      "scene-lab-and-shop-building-004",
      "scene-lab-and-shop-building-005",
      "scene-lab-and-shop-building-006",
      "scene-lab-and-shop-building-007",
      "scene-lab-and-shop-building-008",
      "scene-lab-and-shop-building-009",
      "scene-lab-and-shop-building-010",
      "scene-lab-and-shop-building-011",
      "scene-lab-and-shop-building-012",
      "scene-lab-and-shop-building-013",
      "scene-citd-1st-floor-1",
      "scene-citd-1st-floor-2",
      "scene-citd-1st-floor-3",
      "scene-citd-1st-floor-4",
      "scene-citd-1st-floor-5",
      "scene-citd-1st-floor-6",
      "scene-citd-1st-floor-7",
      "scene-citd-1st-floor-8",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-85",
      "scene-general-road-94",
      "scene-general-road-93",
      "scene-general-road-92",
      "scene-general-road-91",
      "scene-chs-1st-floor-001",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building III",
    destination_node_key: "acad-3",
    arrival_scene_key: "scene-cas-1st-floor",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
      "scene-graduate-school-1st-floor-2",
      "scene-graduate-school-1st-floor-3",
      "scene-graduate-school-1st-floor-4",
      "scene-graduate-school-1st-floor-5",
      "scene-lab-and-shop-building-001",
      "scene-lab-and-shop-building-002",
      "scene-lab-and-shop-building-003",
      "scene-lab-and-shop-building-004",
      "scene-lab-and-shop-building-005",
      "scene-lab-and-shop-building-006",
      "scene-lab-and-shop-building-007",
      "scene-lab-and-shop-building-008",
      "scene-lab-and-shop-building-009",
      "scene-lab-and-shop-building-010",
      "scene-lab-and-shop-building-011",
      "scene-lab-and-shop-building-012",
      "scene-lab-and-shop-building-013",
      "scene-citd-1st-floor-1",
      "scene-citd-1st-floor-2",
      "scene-citd-1st-floor-3",
      "scene-citd-1st-floor-4",
      "scene-citd-1st-floor-5",
      "scene-citd-1st-floor-6",
      "scene-citd-1st-floor-7",
      "scene-citd-1st-floor-8",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-general-road-37",
      "scene-general-road-38",
      "scene-general-road-38-5",
      "scene-general-road-39",
      "scene-cas-1st-floor",
    ]),
  }),
  Object.freeze({
    destination_name: "Supply & Property Building",
    destination_node_key: "supply-property-bldg",
    arrival_scene_key: "scene-supply-1st-floor-001",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
      "scene-graduate-school-1st-floor-2",
      "scene-graduate-school-1st-floor-3",
      "scene-graduate-school-1st-floor-4",
      "scene-graduate-school-1st-floor-5",
      "scene-lab-and-shop-building-001",
      "scene-lab-and-shop-building-002",
      "scene-lab-and-shop-building-003",
      "scene-lab-and-shop-building-004",
      "scene-lab-and-shop-building-005",
      "scene-lab-and-shop-building-006",
      "scene-lab-and-shop-building-007",
      "scene-lab-and-shop-building-008",
      "scene-lab-and-shop-building-009",
      "scene-lab-and-shop-building-010",
      "scene-lab-and-shop-building-011",
      "scene-lab-and-shop-building-012",
      "scene-lab-and-shop-building-013",
      "scene-citd-1st-floor-1",
      "scene-citd-1st-floor-2",
      "scene-citd-1st-floor-3",
      "scene-citd-1st-floor-4",
      "scene-citd-1st-floor-5",
      "scene-citd-1st-floor-6",
      "scene-citd-1st-floor-7",
      "scene-citd-1st-floor-8",
      "scene-general-road-32",
      "scene-supply-1st-floor-001",
    ]),
  }),
  Object.freeze({
    destination_name: "CITD Building",
    destination_node_key: "citd",
    arrival_scene_key: "scene-citd-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
      "scene-graduate-school-1st-floor-2",
      "scene-graduate-school-1st-floor-3",
      "scene-graduate-school-1st-floor-4",
      "scene-graduate-school-1st-floor-5",
      "scene-lab-and-shop-building-001",
      "scene-lab-and-shop-building-002",
      "scene-lab-and-shop-building-003",
      "scene-lab-and-shop-building-004",
      "scene-lab-and-shop-building-005",
      "scene-lab-and-shop-building-006",
      "scene-lab-and-shop-building-007",
      "scene-lab-and-shop-building-008",
      "scene-lab-and-shop-building-009",
      "scene-lab-and-shop-building-010",
      "scene-lab-and-shop-building-011",
      "scene-lab-and-shop-building-012",
      "scene-lab-and-shop-building-013",
      "scene-citd-1st-floor-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Laboratory & Shop Building",
    destination_node_key: "lab-shop",
    arrival_scene_key: "scene-lab-and-shop-building-001",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
      "scene-graduate-school-1st-floor-2",
      "scene-graduate-school-1st-floor-3",
      "scene-graduate-school-1st-floor-4",
      "scene-graduate-school-1st-floor-5",
      "scene-lab-and-shop-building-001",
    ]),
  }),
  Object.freeze({
    destination_name: "Graduate School Building",
    destination_node_key: "graduate",
    arrival_scene_key: "scene-graduate-school-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Technohub Building",
    destination_node_key: "techno-bldg",
    arrival_scene_key: "scene-techno-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building I",
    destination_node_key: "acad-1",
    arrival_scene_key: "scene-acad1-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-3",
      "scene-multi-2-1st-floor-1",
      "scene-multi-2-1st-floor-2",
      "scene-multi-2-1st-floor-3",
      "scene-multi-2-1st-floor-4",
      "scene-acad1-1st-floor-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Multi-Purpose Building II",
    destination_node_key: "multi-2",
    arrival_scene_key: "scene-multi-2-2nd-floor-2",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
    ]),
  }),
  Object.freeze({
    destination_name: "Green Building",
    destination_node_key: "green",
    arrival_scene_key: "scene-green-1st-floor-9",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-multi-2-2nd-floor-1",
      "scene-multi-2-2nd-floor-2",
      "scene-multi-2-2nd-floor-4",
      "scene-multi-2-2nd-floor-5",
      "scene-acad1-2nd-floor-1",
      "scene-acad1-2nd-floor-2",
      "scene-acad1-2nd-floor-3",
      "scene-acad1-2nd-floor-4",
      "scene-acad1-1st-floor-4",
      "scene-acad1-1st-floor-5",
      "scene-acad1-1st-floor-6",
      "scene-acad1-1st-floor-7",
      "scene-techno-1st-floor-1",
      "scene-techno-1st-floor-2",
      "scene-graduate-school-1st-floor-1",
      "scene-graduate-school-1st-floor-2",
      "scene-graduate-school-1st-floor-3",
      "scene-graduate-school-1st-floor-4",
      "scene-graduate-school-1st-floor-5",
      "scene-lab-and-shop-building-001",
      "scene-lab-and-shop-building-002",
      "scene-lab-and-shop-building-003",
      "scene-lab-and-shop-building-004",
      "scene-lab-and-shop-building-005",
      "scene-lab-and-shop-building-006",
      "scene-lab-and-shop-building-007",
      "scene-lab-and-shop-building-008",
      "scene-lab-and-shop-building-009",
      "scene-lab-and-shop-building-010",
      "scene-lab-and-shop-building-011",
      "scene-lab-and-shop-building-012",
      "scene-lab-and-shop-building-013",
      "scene-citd-1st-floor-1",
      "scene-citd-1st-floor-2",
      "scene-citd-1st-floor-3",
      "scene-citd-1st-floor-4",
      "scene-citd-1st-floor-5",
      "scene-citd-1st-floor-6",
      "scene-citd-1st-floor-7",
      "scene-citd-1st-floor-8",
      "scene-general-road-32",
      "scene-general-road-33",
      "scene-general-road-33-5",
      "scene-green-1st-floor-9",
    ]),
  }),
  Object.freeze({
    destination_name: "Staff House",
    destination_node_key: "staff-house",
    arrival_scene_key: "scene-staff-house-2nd-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-staff-house-2nd-floor-1",
      "scene-guard-staff-house-2nd-floor-2",
      "scene-guard-staff-house-2nd-floor-3",
      "scene-guard-staff-house-2nd-floor-4",
      "scene-staff-house-2nd-floor-1",
    ]),
 }),
  Object.freeze({
    destination_name: "Villafuerte Hall",
    destination_node_key: "villafuerte-hall",
    arrival_scene_key: "scene-vh-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-016",
      "scene-audit-building-015",
      "scene-audit-building-014",
      "scene-audit-building-013",
      "scene-audit-building-012",
      "scene-audit-building-006",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-26",
      "scene-general-road-25",
      "scene-pearl-park-16",
      "scene-pearl-park-17",
      "scene-vh-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Pearl Park",
    destination_node_key: "pearl-park",
    arrival_scene_key: "scene-pearl-park-2",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-016",
      "scene-audit-building-015",
      "scene-audit-building-014",
      "scene-audit-building-013",
      "scene-audit-building-012",
      "scene-audit-building-006",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-26",
      "scene-general-road-25",
      "scene-pearl-park-2",
    ]),
  }),
  Object.freeze({
    destination_name: "Freedom Park",
    destination_node_key: "free-park",
    arrival_scene_key: "scene-freedom-park-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-general-road-78",
      "scene-freedom-park-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Academic Building V",
    destination_node_key: "acad-5",
    arrival_scene_key: "scene-acad-5-1st-floor-7",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-016",
      "scene-audit-building-015",
      "scene-audit-building-014",
      "scene-audit-building-013",
      "scene-audit-building-012",
      "scene-audit-building-006",
      "scene-audit-building-005",
      "scene-audit-building-004",
      "scene-audit-building-003",
      "scene-audit-building-002",
      "scene-audit-building-001",
      "scene-acad-2-1st-floor-15",
      "scene-acad-2-1st-floor-20",
      "scene-acad-2-1st-floor-19",
      "scene-acad-2-1st-floor-18",
      "scene-acad-2-1st-floor-16",
      "scene-acad-2-1st-floor-17",
      "scene-general-road-60",
      "scene-general-road-59",
      "scene-general-road-58",
      "scene-general-road-57",
      "scene-general-road-53",
      "scene-general-road-52",
      "scene-general-road-51",
      "scene-general-road-50",
      "scene-general-road-49",
      "scene-acad-5-1st-floor-7",
    ]),
  }),
  Object.freeze({
    destination_name: "Duran Hall",
    destination_node_key: "duran",
    arrival_scene_key: "scene-duran-1st-floor-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-general-road-72",
      "scene-duran-1st-floor-1",
    ]),
  }),
  Object.freeze({
    destination_name: "Gymnasium",
    destination_node_key: "gym",
    arrival_scene_key: "scene-gym-1st-floor-bleacher-1",
    scene_keys: Object.freeze([
      "scene-guard-house-walk-1st-floor-1",
      "scene-guard-house-walk-1st-floor-2",
      "scene-staff-house-1st-floor-1",
      "scene-staff-house-1st-floor-2",
      "scene-admin-1st-floor-5",
      "scene-admin-1st-floor-4",
      "scene-admin-1st-floor-3",
      "scene-admin-1st-floor-2",
      "scene-admin-1st-floor-1",
      "scene-foodlab-1st-floor-5",
      "scene-foodlab-1st-floor-4",
      "scene-foodlab-1st-floor-3",
      "scene-foodlab-1st-floor-1",
      "scene-library-1st-floor-10",
      "scene-library-1st-floor-9",
      "scene-library-1st-floor-5",
      "scene-library-1st-floor-4",
      "scene-library-1st-floor-3",
      "scene-library-1st-floor-2",
      "scene-library-1st-floor-1",
      "scene-audit-building-019",
      "scene-audit-building-017",
      "scene-audit-building-016",
      "scene-audit-building-015",
      "scene-audit-building-014",
      "scene-audit-building-013",
      "scene-audit-building-012",
      "scene-audit-building-006",
      "scene-general-road-64",
      "scene-general-road-65",
      "scene-general-road-26",
      "scene-general-road-25",
      "scene-gym-1st-floor-bleacher-1",
    ]),
  }),
]);

// Legacy consumers intentionally continue to receive the Vehicle catalog.
const GUIDED_VR_ROUTES = VEHICLE_GUIDED_VR_ROUTES;
const GUIDED_VR_ROUTES_BY_MODE = Object.freeze({
  vehicle: VEHICLE_GUIDED_VR_ROUTES,
  walking: WALKING_GUIDED_VR_ROUTES,
});
const GUIDED_VR_EXIT_ROUTES_BY_MODE = Object.freeze({
  vehicle: VEHICLE_EXIT_GUIDED_VR_ROUTES,
  walking: WALKING_GUIDED_VR_ROUTES,
});
const DEFERRED_GUIDED_VR_DESTINATIONS = Object.freeze([]);

module.exports = {
  VEHICLE_GUIDED_VR_ROUTES,
  VEHICLE_EXIT_GUIDED_VR_ROUTES,
  WALKING_GUIDED_VR_ROUTES,
  GUIDED_VR_ROUTES,
  GUIDED_VR_ROUTES_BY_MODE,
  GUIDED_VR_EXIT_ROUTES_BY_MODE,
  DEFERRED_GUIDED_VR_DESTINATIONS,
};
