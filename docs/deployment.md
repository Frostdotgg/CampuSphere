...
## Current deployment handoff (2026-09-22)
...
`dpl_HbJRojVKtJUeUr2KMY6XhcRv6RCS` on
`https://campusphere-cspc.vercel.app`. Documentation/test commits `26f8cc6`
and `3d43650` were deliberately not promoted; the root `.vercelignore`
allowlist excludes their non-runtime surfaces by default. The retained LT-08 window showed zero
function errors, zero function timeouts, no displayed `5xx`, and memory well
below the provisioned limit, but it contained three session-store retry
warnings and is not a zero-warning window. This documentation checkpoint does
not authorize a Vercel deployment or promotion. Supabase sources remain
contiguous through owner-reported-applied migration `0027`; never reapply an
owner-applied migration without fresh explicit database authority.
MySQL synchronization remains required later but is not the immediate next
move and is not authorized by grounding.

The correlated September 14 session-readiness incident evidence is recorded in
`docs/session-readiness-incident-2026-09-14.md`. It confirms a Supabase `401`
for the exact readiness query at 21:29:01 Asia/Manila, but the retained row does
not contain the provider reason for that temporary authorization response.
...
Offline and Last seen view, where Online means a timestamp within the inclusive
five-minute window. The `user_presence` table is separate from
`users.updated_at`, contains no IP/device/session history, and is never
backfilled. MySQL receives the additive table through
`node scripts/applyUserPresenceMysqlMigration.js`; Supabase migration
`0022_user_presence.sql` has been applied by the owner after the application
security/performance review. Read-only table/API postflight checks succeeded;
the owner-controlled SQL Editor remains the authority for catalog metadata.
...
ordered postconditions are `24/24 -> 18/18 -> 46/46`. The historical source/product
commits are `fdb0c8c`, `621d72e`, `b8e7ffb`, and `a5a6cee`; the authority synchronization is the
live `HEAD` after delivery. That 2026-09-06 push was source delivery only and
included no Vercel/ICTU deployment, promotion, or Production smoke; the current
2026-09-07 block records the owner's later promotion report separately.
...
not apply or reapply it. The application container was rebuilt after the source
changes with the external Supabase env file and returned HTTP 200 from
`/healthz`. Runtime/source commit `5d505e9` is followed by pushed authority
successor `d294cfd`. The current route-color source package is 199 files,
7,344,623 bytes, aggregate SHA-256
`9420ce6a273e6ce52856936c7343efe4b864f23df17e030a0ac5b184595f2d4e`; the
earlier 197-file package remains historical.
...
product commit `b8d2bf26a73d25cff2c53af695396d2f695ae631` (`b8d2bf2`; completed-response
...
product commit `13adb9da0e3ff01b6ed853ed1f993c3d2a12a207` (`13adb9d`; recoverable Production session
...
`HEAD`, `origin/main`, and remote `main` equal at Git commit SHA-1
...
catalog contains 25 active destinations, 484 configured steps, and 100 unique
...
The current Windows Supabase-backed local rehearsal uses the same testing
compose file with an external, untracked env file. Use the file; never open,
print, summarize, or commit its values:

```powershell
docker compose --env-file "C:\campusphere-secrets\campusphere-local-supabase.env" -f docker-compose.testing.yml up -d --build
docker compose --env-file "C:\campusphere-secrets\campusphere-local-supabase.env" -f docker-compose.testing.yml ps
