...
## Current deployment handoff (2026-09-14)

Use `docs/current-authority.md` for the canonical release/evidence snapshot.
Current pushed product source is `4e9d579`, after `13ae67c` and `f9679f6`. The owner
promoted it, and a signed-in Vercel dashboard observation showed the matching
`main` deployment `Ready` in `Production` on
`https://campusphere-cspc.vercel.app`. Corrected anonymous GET-only Production
smoke passed `207/207`; bounded guest UAT opened 25/25 online and 25/25 offline
building panels. Sampled deployed assets matched committed Git blobs, but this
does not prove complete immutable-package equality. Full `npm test` and current
MySQL parity were deferred for this release. Supabase sources remain contiguous
through owner-reported-applied migration `0027`; never reapply an owner-applied
migration without fresh explicit database authority.
