...
## Current route/media demonstration note (2026-09-22)

For a controlled demonstration, an administrator may draw one directed edge
and observe Distance and Walktime fill from its geometry; entry and exit remain
separate. Entry lines are blue and exit lines are red in both online and
offline route views; written direction labels remain the primary cue. Use
disposable/local data unless the owner explicitly authorizes a live-data
demonstration. For media, paste an approved Cloudinary URL or a
Google Drive single-file link shared as viewer. Use JPEG, PNG, or WebP; HEIC is
not supported. Delete disposable demonstration content through the normal UI.
These actions are not authorized merely by this script.

Current source also contains explicit Vehicle exit Guided-VR sequences: 25
...
690 steps / 133 unique scenes. Read `docs/current-authority.md` before a demo;
the retained load-test closeout does not authorize Production writes.
Immediately before the current authority-only update, clean
local/upstream/remote `main` matched `3d43650`; Production remains the
owner-observed `0e7fe8b` deployment. MySQL synchronization remains required
later but is not the immediate next move.
...
Current deployment evidence is recorded in `docs/current-authority.md`:
owner-promoted release `b8d2bf2` and deployment
...
later bounded authenticated guest observation kept repeated heartbeat
responses at `204` during intermittent session-touch timeout/retry diagnostics;
the signed-in page remained usable, with no `5xx` or header-sent error. It was
not a zero-diagnostic result, induced outage, or full signed-in UAT. The
signed-in 25/25 online/offline guest UAT remains predecessor
evidence for `4e9d579` and was not rerun. Neither result proves every VR
journey or 2D route, disconnected cold reload, admin/OAuth/schedule/real Drive
media, an induced Supabase outage, or complete immutable deployed-package
equality. This script does not authorize a live-data demonstration.

For questions about the former fixed 503 JSON, use the source-backed, panel-ready
wording in `docs/session-readiness-incident-2026-09-14.md`: the retained log
confirms a Supabase `401` on the readiness request, while the provider's reason
for issuing that temporary `401` is not present in the retained row.
...
order, and VR navigation uses admin-managed non-private data. MySQL freezes 34
  buildings, 44 route nodes, 100 directed edges, 50 reverse pairs, 25 exact
