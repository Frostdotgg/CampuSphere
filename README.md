...
- Administrator-drawn directional edge geometry automatically supplies rounded
  Haversine distance and 1.2 m/s walk-time metrics without deleting/recreating
  existing routes.
- Building, room-schedule, and 360-scene media may reference approved
  Cloudinary or Google Drive files; Drive images use an authenticated
  same-origin proxy.
...
- **Maps/VR:** Leaflet, MapLibre GL JS, OpenStreetMap/PMTiles, Pannellum
- **Media delivery:** validated Cloudinary or Google Drive references
- **Security:** role authorization, CSRF, rate limiting, Supabase RLS/server-only
  grants, nonce-based CSP, and Helmet security headers
- **Offline:** service worker, IndexedDB package management, and PMTiles
- **Deployment:** Vercel demo/UAT and Docker Compose/ICTU arrangements
- **Config:** `dotenv`
...
├── docs/                # current authority, handoff, deployment, evidence
├── middleware/          # login/role, CSRF, rate limits, security headers
...
#### Admin-managed instructor profiles (pushed source release, 2026-09-07)
...
The selected Supabase project has owner-applied migrations `0001`-`0027`.
Codex did not apply or reapply them. Do not reapply an owner-applied migration
without fresh explicit database authorization.

The schedule history includes `0012_room_schedules` and
`0013_vr_hotspot_schedule_metadata`; the current semester-image workflow is
`0020_room_schedule_documents`, followed by the owner-applied presence,
directional-route, visibility, audience, instructor-profile, and route-metric
migrations through `0027`.
...
2026-09-06 record measured MySQL at 34 buildings, 44 route nodes, 100 directed
edges, 50 reverse pairs, 50 exact reverse geometries, and 100 valid geometries;
Supabase at 25 buildings, 26 route nodes, 50 directed edges, 25 reverse pairs,
0 exact reverse geometries, and 50 valid geometries. Intentional later
...
- **Cloudinary** and **Google Drive** delivery URLs are administrator-supplied metadata and documented in the media subsection below.
...
CampuSphere delivers building pictures, semester room-schedule images, and 360-degree VR panoramas through either **Cloudinary** or **Google Drive**. Administrators paste a validated delivery link into `image_url`; `cloudinary_public_id` is optional metadata for Cloudinary only. Cloudinary media remains on `https://res.cloudinary.com`. Drive media is rendered through the authenticated same-origin `/api/media/google-drive/:fileId` proxy, so browser pages never load an arbitrary Drive URL directly. The application does not upload, delete, or manage vendor assets. Google-managed profile photos remain read-only and manual profile-photo upload is deferred. When no remote asset is configured the app falls back to local `/img/*` and `/img/vr/*` placeholders.

Rules:

- Cloudinary delivery URLs and exact Google Drive single-file share URLs are validated server-side by `utils/mediaUrl.js`; only the approved provider forms or local placeholders are accepted.
- A Drive file must be shared as **Anyone with the link · Viewer**. Folder links, fragments, unsupported query parameters, and look-alike hosts are rejected. The proxy follows only approved Google HTTPS redirects, enforces image signature/MIME checks and a 50 MiB limit, and sends private/no-store or short private caching headers.
- `cloudinary_public_id` is administrator/server metadata only, applies only to a Cloudinary URL, and never appears in public/runtime responses. Selecting Drive clears stale Cloudinary metadata.
- Campus/VR/schedule asset upload, sharing, and credential entry are owner-controlled outside the app. Cloudinary delivery remains owner-controlled; CampuSphere has no browser direct-upload, Drive OAuth, unsigned-upload preset, SDK write, Cloudinary Admin API, vendor management API, or asset-delete flow.
- Room-schedule images and Drive-backed media are online-only and are not copied into offline-guide IndexedDB packages.
