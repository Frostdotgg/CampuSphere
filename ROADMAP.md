...
Use `docs/current-authority.md` for current release truth. The roadmap's older
milestone and continuity sections are retained as historical planning evidence
when they conflict with this status.

- Automatic route distance/walk-time calculation from directed geometry is
  complete in `918f721`; owner-applied migration `0027` and owner-accepted
  route/data UAT are recorded without claiming a new immutable route freeze.
- Cloudinary-or-Google-Drive media references are complete in `83f247a`;
  Drive is authenticated reference delivery, not file upload/management.
- Entry/exit route colors are complete in `3d0a2b6`, with pushed authority at
  `7b4e818`. Deployment `dpl_CG3M2Wp4hdMUR1abBFJdv5mqgtNs` is independently
  verified Ready/Production/Current.
- Fresh full `npm test` passed with `QUALITY-GATES OK`, final residue passed
  `18/18`, and the corrected bounded Production smoke passed `127/127`.
- The current work is owner-continuity authority synchronization. After review
  and separate commit/push authorization, proceed to another owner-selected
  bug fix or feature without reapplying `0027` or changing final routes.

## Historical Room Schedule Image Change (2026-08-25; superseded)

This historical source candidate changed room scheduling to one semester-long image
...
packages continue to exclude schedules. At that historical point migration
source `0020` existed but was not applied; the current authority records
`0020` through `0026` as owner-applied.
...
The runtime/source release is committed and pushed as
`5d505e97e990ad82df6c858e28d48542deb8bf2c` (`5d505e9`) on `main`;
pushed authority successor `d294cfd` was the clean synchronized HEAD at the
start of this authority update. The reviewed Vercel package is 197 files,
...
independent deployment identity and Production verification remain pending.
