...
- Entry/exit route colors are complete in `3d0a2b6`, with pushed authority at
  `8e6053e`; the owner reports that release promoted.
- The current work is owner-continuity authority synchronization. After review
  and separate commit/push authorization, the product-quality sequence is
  independent `8e6053e` deployment verification, supported end/expiry of the
  one observed Supabase admin session and an authorized gate rerun, a bounded
  Production smoke, and then another owner-selected task.
