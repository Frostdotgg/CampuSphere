...
Pushed and owner-promoted release `b8d2bf2` guards completed responses against
the late session-touch failure that previously produced
`ERR_HTTP_HEADERS_SENT` after a heartbeat `204`. Deployment
`dpl_5aBjCeWeBj1ZcST2LJCqZhSv7Tct` passed bounded anonymous Production smoke
`301/301`; its filtered logs contained no `5xx` or header-sent error. The
predecessor 25/25 online/offline guest UAT was not rerun. Current MySQL parity
and full dual-backend testing remain deferred. The next release-closeout move
is authority-diff review and static checks, then separately authorized commit
and push if green. Older milestone and target-stack
