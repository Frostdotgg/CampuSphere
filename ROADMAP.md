...
Immediately before this authority synchronization, clean local/upstream/remote
`main` matched `3d43650` with ahead/behind `0/0` and zero stashes. The
owner-observed Vercel Production deployment remains
`dpl_HbJRojVKtJUeUr2KMY6XhcRv6RCS` for product commit `0e7fe8b`; later
documentation/test commits were deliberately not promoted. The retained
Production load suite closes
LT-01 through LT-08 with explicit limitations: LT-01 through LT-06 passed,
LT-07 passed through authenticated Chrome while its preceding k6 result remains
rejected harness evidence, and LT-08 is **PASS WITH WARNING** after all 26
thresholds passed. Current source includes explicit Vehicle exits and service
worker `v48`. The owner has now authorized one dedicated authority commit and a
normal push to `main`. After that push, start the next owner session with
read-only grounding before selecting one focused feature or bug fix. MySQL
synchronization remains required later but is not the next move. Older milestone
and target-stack sections below are historical where they conflict with this
status and `docs/current-authority.md`.
