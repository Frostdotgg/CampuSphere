...
Pushed authority commit `58298c9` records the owner-promoted `13adb9d` release,
which adds recoverable fail-closed session
readiness and bounded Supabase session-store retries without changing the
complete Walking/MapLibre/route product behavior from `4e9d579`. Its bounded
anonymous Production smoke passed `301/301`; the predecessor 25/25 online/
offline guest UAT was not rerun. A retained Supabase log confirms the September
14 readiness request received `401`; see the dedicated incident record. The
next focused product milestone is a read-only diagnosis of the separate
presence-heartbeat double-response observation. Current MySQL parity and
full dual-backend testing remain deferred. Older milestone and target-stack
sections below are historical where they conflict with this status and
