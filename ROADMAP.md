...
The owner-promoted `4e9d579` release contains the complete Walking Guided-VR
catalog, Walking exits, travel-mode chooser, MapLibre online/offline maps, and
the latest Academic IV/VI route decisions. Corrected Production smoke passed
`207/207` and bounded guest UAT passed for 25/25 online and offline building
panels. The next product milestone is chosen by the owner from bug fixes or
add/change/remove features. Current MySQL parity and full dual-backend testing
remain deferred. Older milestone and target-stack sections below are retained
as historical planning evidence where they conflict with this status and
