...
## Current Progress and Next Moves (2026-09-22)

Immediately before this authority synchronization, clean local `HEAD`,
`origin/main`, and remote `main` matched `3d43650` with ahead/behind `0/0` and
zero stashes. The owner-observed current Vercel Production deployment remains
`dpl_HbJRojVKtJUeUr2KMY6XhcRv6RCS` for product commit `0e7fe8b`; later
documentation/test commits were deliberately not promoted.
Retained LT-01 through LT-06 passed; LT-07 passed through the owner's
authenticated Chrome profile while the earlier k6 result remains rejected
harness evidence; and LT-08 passed all 26 configured thresholds with a **PASS
WITH WARNING** disposition because three session-store retry warnings appeared
in the matching Vercel window. Current source has 25 Vehicle entry, 25 Vehicle
exit, and 25 Walking destinations, and service worker `v48`.

The owner has now authorized review, bounded validation, one dedicated authority
commit, and a normal push to `main`. No deployment, promotion, database,
migration, or vendor action is authorized. After that push, open a new owner
session for read-only grounding, select one focused feature or bug fix, and
isolate it on a fresh branch from then-current clean `main`. MySQL
synchronization remains required later but is not the next move.
Older sections below are historical where they conflict with this status and
`docs/current-authority.md`.

## Historical Pushed Room Schedule Image Candidate (2026-08-26; superseded)

The authorized implementation is verified, reviewed, committed, and pushed as
`e481d03`. It replaces legacy time-row
administration with one stable `room_schedule_documents` record per
building/room/floor identity. Admins update that record's semester, school year,
and externally supplied Cloudinary delivery URL; building details and VR
hotspots read the same record through a shared accessible viewer. Legacy rows
are read-only fallback data, and offline scope is unchanged.

Migration source `0020_room_schedule_documents.sql` is owner-applied in the
selected Supabase and local MySQL stores. Runtime dual-backend checks, review,
commit, and push are complete evidence; deployment and Production acceptance
remain separate authorization/evidence boundaries.

## Guest building/VR visibility and event-audience policy (pushed source release, 2026-09-07)

Signed-in guests can browse every building, 2D route, and 360 scene. Schedule
APIs and schedule hotspots remain limited to students, instructors, and admins;
scene/exit hotspots remain visible to guests, while information hotspots require
an explicit admin approval bit. MySQL schema/seed support the policy and
Supabase migrations `0024_vr_hotspot_guest_visibility.sql` and
`0025_event_audience.sql` are owner-applied; Codex did not apply or reapply
them. Do not reapply them without fresh explicit database
authorization. The selected 2026-09-06 freeze is historical after later
intentional route/metric changes; see `docs/current-authority.md`.

## Admin-managed instructor profile integrity (pushed source release, 2026-09-07)

The admin user flow now keeps instructor role profiles complete on both
backends. A new instructor and an account promoted to instructor receive one
minimal `instructor_profiles` row (`employee_id`, `department`, and `position`
blank; `status = 'Active'`) when none exists. Existing profile values are
preserved, role changes away from instructor are non-destructive, and a
conflict-safe backfill repairs only missing rows. MySQL performs the profile
insert in the same transaction as the user write; Supabase uses the
server-only atomic admin RPCs in migration
`0026_admin_instructor_profile_integrity.sql`, which the project owner has
applied. Codex did not apply or reapply it. This auth/profile change does not alter the selected campus,
...
HEAD at the start of this authority update. The reviewed Vercel package is
197 files, 7,301,960 bytes, aggregate SHA-256
...
while exact deployment identity, Production smoke, and immutable deployed-byte
verification remain pending independent evidence.

## Historical pre-918f721 summary (superseded)
