...
/*
 * Narrow, owner-authorized Supabase repair for the two remaining Walking
 * Guided VR arrival scenes that are present but are not linked to a route node.
 *
 * The default mode is a read-only preflight. Pass --apply to update only the
 * two vr_scenes.node_id values. Vehicle arrival mappings, media, hotspots, and
 * every other scene field are deliberately outside the write.
 */
...
const TARGETS = Object.freeze([
  Object.freeze({
    ...
    arrival_scene_key: 'scene-freedom-park-1'
  }),
  Object.freeze({
    destination_name: 'Duran Hall',
    destination_node_key: 'duran',
    arrival_scene_key: 'scene-duran-1st-floor-1'
  })
]);
...
function validateCatalog() {
  check(TARGETS.length === 2, 'repair target count is not two');
...
function buildPlan(state) {
  ...
  for (const target of TARGETS) {
    ...
    plan.push({
      ...
      scene,
      node,
      vehicleArrivalSceneKey: vehicleRoute.arrival_scene_key,
...
function verifyPostflight(before, after) {
  ...
  for (const target of TARGETS) {
    ...
    check(node && afterByScene.get(target.arrival_scene_key) === node.id,
