...
/* Pure safety tests for the two-target Walking arrival repair helper. */
...
const {
  ...
  applyPlan,
  verifyPostflight
} = require('./repairRemainingWalkingVrArrivalMappings');
const {
  VEHICLE_GUIDED_VR_ROUTES
} = require('../config/guidedVrRoutes');

function route(nodeKey) {
  return VEHICLE_GUIDED_VR_ROUTES.find((entry) => entry.destination_node_key === nodeKey);
}
...
function validState({ current = 'unassigned' } = {}) {
  ...
  const vehicleScenes = TARGETS.map((target, index) => ({
    scene_key: route(target.destination_node_key).arrival_scene_key,
    node_id: index + 101
  }));
...
(async () => {
  ...
  assert.equal(plan.filter((item) => item.current === 'unassigned').length, 2);
  ...
  assert.equal(skipped.filter((item) => item.result === 'already-correct').length, 2);
  ...
  console.log('REPAIR-REMAINING-WALKING-ARRIVAL-PROBE OK: guard, conflict, concurrency, skip, and stop-on-error cases passed.');
})().catch((error) => {
  console.error(`REPAIR-REMAINING-WALKING-ARRIVAL-PROBE FAILED: ${error.message || 'synthetic check failed'}`);
  process.exitCode = 1;
});
