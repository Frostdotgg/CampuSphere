...
`b8d2bf2` with a clean tree and zero stashes. Product release `b8d2bf2` is
owner-promoted as `dpl_5aBjCeWeBj1ZcST2LJCqZhSv7Tct`; its bounded anonymous
Production smoke passed `301/301`, and deployment-specific logs contained no
`5xx` or `ERR_HTTP_HEADERS_SENT`. It prevents a late session-touch failure from
writing a second response after a completed `204`. The earlier signed-in 25/25
online/offline guest UAT remains predecessor evidence for `4e9d579` and was not
rerun. Full `npm test` and current MySQL parity remain deferred.
