...
start of this authority synchronization, clean local/upstream/remote
`main` matched `3d43650` with ahead/behind `0/0` and zero stashes.
The owner-observed current Production deployment is
`dpl_HbJRojVKtJUeUr2KMY6XhcRv6RCS` for product commit `0e7fe8b`; later
documentation/test commits were deliberately not promoted. Retained
LT-01 through LT-06 passed; LT-07 passed through the owner's authenticated
Chrome profile while the preceding k6 result remains rejected harness evidence;
and LT-08 passed all 26 configured thresholds with a **PASS WITH WARNING**
disposition because three session-store timeout-retry warnings appeared in the
matching Vercel window. Current source has 25 Vehicle entry / 25 Vehicle exit /
25 Walking destinations and service worker `v48`. Current MySQL parity and
complete deployed-byte equality remain unestablished. The owner has now
authorized one dedicated authority commit and a normal push to `main`. After
that push, start a new owner session with read-only grounding before selecting
one focused feature or bug fix; MySQL synchronization remains required later
but is not the next move.
...
schedules are limited to `student-cspc`, `instructor`, and `admin`; VR scene and
exit hotspots remain guest-visible, while information hotspots require an
explicit administrator approval bit and schedule hotspots are always hidden.
The MySQL schema/seed and Supabase migrations `0024_vr_hotspot_guest_visibility.sql`
and `0025_event_audience.sql` carry this policy. Migrations 0024 and 0025 are
owner-applied in Supabase; Codex did not apply or reapply them. Do not reapply
them without fresh explicit database
authorization; the selected historical freeze is unchanged.
...
`instructor`. Legacy employee/department/position fields remain blank and the
status is `Active`; existing profile values are preserved and role changes away
from instructor do not delete profiles. MySQL performs the ensure inside the
user transaction. Supabase migration `0026_admin_instructor_profile_integrity.sql`
...
`231/231`, dedicated ports `3383`/`3384`), and
...
Migrations `0020` through `0026` are owner-applied; Codex did not apply or
reapply `0026_admin_instructor_profile_integrity.sql`.
...
candidate. The current source package pin is 197 files, 7,301,960 bytes,
