...
## Room Schedule Image Candidate (2026-08-25)

A separately authorized feature implementation was merged from the isolated
worktree `C:\Users\FROST.GG\Desktop\CampuSphere-room-schedule-images` on
branch `feat/room-schedule-images`, based on commit
`2b4f42df3f79347c70af07f7b98f70be55b701bd`. The original release-authority,
profile-photo, and PWA changes remain in this worktree.

The candidate replaces legacy time-row administration with one current
semester schedule image record per room/facility. Admins paste an approved,
participant-PII-free HTTPS Cloudinary delivery URL and optional public ID;
CampuSphere never uploads, transforms, or deletes the asset. Building details
and VR schedule hotspots open one shared accessible image viewer. A hotspot
stores `schedule_document_id`, so editing the existing room record for a new
semester updates every linked hotspot. Legacy rows and hotspot metadata remain
read-only fallback data, and schedules remain outside offline packages.

Supabase migration source `0020_room_schedule_documents.sql` is owner-applied
in Supabase; the matching MySQL schema is present for local verification.
Dual-backend runtime CRUD verification, commit, push, deployment, and
Production acceptance remain separate authorization/evidence boundaries.
