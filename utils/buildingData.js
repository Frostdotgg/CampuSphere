...
const { resolveMediaUrlForBrowser } = require('./mediaUrl');
...
function normalizeBuildingRow(row) {
  ...
  return {
    ...
    // details.img, then the local default. Drive URLs are converted to the
    // authenticated same-origin media proxy here; Cloudinary/local values stay
    // unchanged. An unsafe or malformed stored value falls through instead of
    // rendering. Output key stays `img` so views / public JS are unaffected.
    img:         resolveMediaUrlForBrowser(row.image_url) || resolveMediaUrlForBrowser(details.img) || DEFAULT_IMG,
